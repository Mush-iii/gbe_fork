    ...
    stats(Steam_Overlay_Stats(settings))
{
    ...
    show_user_info = false; // user info panel removed from overlay UI
...
void Steam_Overlay::overlay_state_hook(bool ready)
{
    ...
    if (ready) {
        ...
        if (ImGui::GetCurrentContext() && late_init_imgui.compare_exchange_weak(not_yet, true)) {
            ...
            // Modern theme: built-in ImGui API only (no new fonts/images/assets),
            // so this can't introduce any loading/compatibility issues.
            ...
            colors[ImGuiCol_ChildBg]            = ImVec4(0.11f, 0.12f, 0.14f, 0.60f);
            ...
            colors[ImGuiCol_Button]             = ImVec4(0.20f, 0.45f, 0.85f, 0.75f);
            colors[ImGuiCol_ButtonHovered]      = ImVec4(0.26f, 0.53f, 0.95f, 0.90f);
            colors[ImGuiCol_ButtonActive]       = ImVec4(0.16f, 0.38f, 0.75f, 1.00f);
            colors[ImGuiCol_Header]             = ImVec4(0.20f, 0.45f, 0.85f, 0.55f);
            colors[ImGuiCol_HeaderHovered]      = ImVec4(0.26f, 0.53f, 0.95f, 0.75f);
            colors[ImGuiCol_HeaderActive]       = ImVec4(0.20f, 0.45f, 0.85f, 0.90f);
            ...
            colors[ImGuiCol_TabHovered]         = ImVec4(0.26f, 0.53f, 0.95f, 0.80f);
            ...
            colors[ImGuiCol_ScrollbarGrabActive] = ImVec4(0.20f, 0.45f, 0.85f, 1.00f);
            colors[ImGuiCol_Separator]          = ImVec4(0.24f, 0.26f, 0.30f, 0.60f);
        }
    }
}
...
void Steam_Overlay::render_main_window()
{
    ...
    // Floating, movable, resizable achievement tracker window -- this overlay is now
    // solely an achievement app, not a fullscreen HUD. First-use defaults only, the
    // user is free to move/resize it afterwards like any normal window.
    ImGui::SetNextWindowSize(ImVec2(ImGui::GetFontSize() * 26.0f, ImGui::GetFontSize() * 34.0f), ImGuiCond_FirstUseEver);
    ImGui::SetNextWindowPos(ImVec2(ImGui::GetFontSize() * 4.0f, std::max(ImGui::GetFontSize() * 4.0f, io.DisplaySize.y * 0.08f)), ImGuiCond_FirstUseEver);
    ImGui::SetNextWindowSizeConstraints(ImVec2(ImGui::GetFontSize() * 20.0f, ImGui::GetFontSize() * 18.0f), ImVec2(8192, 8192));
    ImGui::SetNextWindowBgAlpha(1.0f);
    if (ImGui::Begin("PULS3 Achievement Tracker", &show, ImGuiWindowFlags_NoCollapse)) {

        size_t unlocked_count = 0;
        for (auto &a : achievements) {
            if (a.achieved) ++unlocked_count;
        }
        float ach_frac = achievements.empty() ? 0.0f
            : (float)unlocked_count / (float)achievements.size();

        // --- Header card: title, unlocked/total, % chip, progress bar ---
        {
            ImDrawList* dl = ImGui::GetWindowDrawList();
            ...
            const float title_h = ImGui::GetFontSize() * 1.3f;
            const float sub_h = ImGui::GetFontSize() * 1.3f;
            const float bar_h = 6.0f;
            const float rounding = settings->overlay_appearance.notification_rounding;
            const float card_h = pad * 2.0f + title_h + sub_h + bar_h + ImGui::GetFontSize() * 0.5f;

            dl->AddRectFilled(card_p0, ImVec2(card_p0.x + card_w, card_p0.y + card_h),
                IM_COL32(24, 28, 34, 255), rounding);
            dl->AddRect(card_p0, ImVec2(card_p0.x + card_w, card_p0.y + card_h),
                IM_COL32(58, 64, 74, 200), rounding);

            // Title (top-left)
            ImGui::SetCursorScreenPos(ImVec2(card_p0.x + pad, card_p0.y + pad));
            ImGui::TextColored(ImVec4(0.98f, 0.82f, 0.30f, 1.0f), "%s", translationAchievementWindow[current_language]);

            // Unlocked/total count (left, below title)
            char summary_buf[64];
            snprintf(summary_buf, sizeof(summary_buf), "%zu / %zu unlocked", unlocked_count, achievements.size());
            ImGui::SetCursorScreenPos(ImVec2(card_p0.x + pad, card_p0.y + pad + title_h));
            ImGui::TextDisabled("%s", summary_buf);

            // Percentage chip (right, below title)
            char pct_chip[16];
            snprintf(pct_chip, sizeof(pct_chip), "%.0f%%", ach_frac * 100.0f);
            const ImVec2 chip_sz = ImGui::CalcTextSize(pct_chip);
            const ImVec2 chip_pad(8.0f, 3.0f);
            const ImVec2 chip_max(card_p0.x + card_w - pad, card_p0.y + pad + title_h + sub_h - 2.0f);
            const ImVec2 chip_min(chip_max.x - chip_sz.x - chip_pad.x * 2.0f, chip_max.y - chip_sz.y - chip_pad.y * 2.0f);
            const ImU32 chip_col = ach_frac >= 1.0f ? IM_COL32(72, 199, 116, 235) : IM_COL32(48, 108, 220, 235);
            dl->AddRectFilled(chip_min, chip_max, chip_col, (chip_max.y - chip_min.y) * 0.5f);
            ImGui::SetCursorScreenPos(ImVec2(chip_min.x + chip_pad.x, chip_min.y + chip_pad.y));
            ImGui::TextColored(ImVec4(1, 1, 1, 1), "%s", pct_chip);

            // Progress bar along the bottom of the card
            const ImVec2 bar_min(card_p0.x + pad, card_p0.y + card_h - pad * 0.8f - bar_h);
            const ImVec2 bar_max(card_p0.x + card_w - pad, card_p0.y + card_h - pad * 0.8f);
            dl->AddRectFilled(bar_min, bar_max, IM_COL32(255, 255, 255, 25), bar_h * 0.5f);
            const float fill_x = bar_min.x + (bar_max.x - bar_min.x) * ach_frac;
            if (fill_x > bar_min.x) {
                dl->AddRectFilled(bar_min, ImVec2(fill_x, bar_max.y),
                    ach_frac >= 1.0f ? IM_COL32(72, 199, 116, 255) : IM_COL32(64, 150, 255, 255), bar_h * 0.5f);
            }

            // Register the card's full extent so the window can size around it.
            ImGui::SetCursorScreenPos(ImVec2(card_p0.x, card_p0.y + card_h));
            ImGui::Dummy(ImVec2(card_w, ImGui::GetFontSize() * 0.5f));
        }

        // --- HUD toggles, fit in the top part of the overlay ---
        {
            bool any = settings->overlay_show_checkbox_fps || settings->overlay_show_checkbox_frametime ||
                settings->overlay_show_checkbox_playtime;
        ...
        if (achievements.empty()) {
            ImGui::Spacing();
            ImGui::TextDisabled("%s", translationNoUnlockedAchievements[current_language]);
        } else {
            ...
            // Lambda to render a single achievement as a rounded card (icon + text over
            // a background rectangle, drawn using the draw-list channel splitter -- a
            // core ImGui API already linked into this build, no extra resources needed).
            auto render_ach = [this](Overlay_Achievement &x) {
                ...
                ImDrawList* dl = ImGui::GetWindowDrawList();
                const float card_rounding = settings->overlay_appearance.notification_rounding;
                const float card_pad = ImGui::GetFontSize() * 0.5f;
                const ImVec2 card_p0 = ImGui::GetCursorScreenPos();
                const float card_w = ImGui::GetContentRegionAvail().x;

                // channel 0 = card background (drawn after we know the row height),
                // channel 1 = actual widgets, drawn first so we can measure them.
                dl->ChannelsSplit(2);
                dl->ChannelsSetCurrent(1);

                ImGui::SetCursorScreenPos(ImVec2(card_p0.x + card_pad, card_p0.y + card_pad));
                ImGui::BeginGroup();
                ImGui::PushItemWidth(card_w - card_pad * 2.0f);
                        ...
                        if (icon_rsrc->GetResourceId() != 0) {
                            ...
                            if (achieved && x.unlock_percentage >= 0.0f && x.unlock_percentage <= 10.0f) {
                                ...
                                for (int i = steps; i >= 1; --i) {
                                    ...
                                    dl->AddRect(lo, hi, IM_COL32(218, 165, 32, a), card_rounding + expand, 0, 2.0f);
                                }
                                dl->AddRect(icon_min, icon_max,
                                            IM_COL32(255, 215, 90, 220), card_rounding, 0, 2.0f);
                                dl->PopClipRect();
                            }
                        }
                ...
                if (hidden) {
                    ...
                    bool show_desc = ImGui::IsItemActive();
                ...
                if (achieved) {
                    ...
                    ImGui::TextColored(ImVec4(0.42f, 0.78f, 0.46f, 1.0f), translationAchievedOn[current_language], buffer);
                } else {
                    ImGui::TextColored(ImVec4(0.85f, 0.35f, 0.35f, 1.0f), "%s", translationNotAchieved[current_language]);
                }
                ...
                const ImVec2 card_p1 = ImVec2(card_p0.x + card_w, ImGui::GetItemRectMax().y + card_pad);

                dl->ChannelsSetCurrent(0);
                dl->AddRectFilled(card_p0, card_p1, IM_COL32(30, 34, 40, 255), card_rounding);
                dl->AddRect(card_p0, card_p1, IM_COL32(54, 60, 70, 160), card_rounding);

                ImU32 accent = (achieved && x.unlock_percentage >= 0.0f && x.unlock_percentage <= 10.0f)
                    ? IM_COL32(255, 200, 70, 255)
                    : (achieved ? IM_COL32(72, 199, 116, 255) : IM_COL32(90, 96, 106, 255));
                dl->AddRectFilled(card_p0, ImVec2(card_p0.x + 3.0f, card_p1.y), accent, card_rounding);

                dl->ChannelsMerge();

                ImGui::SetCursorScreenPos(ImVec2(card_p0.x, card_p1.y + ImGui::GetFontSize() * 0.4f));
                // Submit a zero-height item at the new cursor position so ImGui can
                // register the extended content bounds (required whenever
                // SetCursorScreenPos moves past the previous max bounds).
                ImGui::Dummy(ImVec2(card_w, 0.0f));
                ImGui::PopID();
            };

            // --- Unlocked section ---
            {
                char unlocked_hdr[96];
                snprintf(unlocked_hdr, sizeof(unlocked_hdr), "%s  (%zu)", translationUnlocked[current_language], unlocked_idx.size());
                if (ImGui::CollapsingHeader(unlocked_hdr, settings->overlay_appearance.unlocked_expanded ? ImGuiTreeNodeFlags_DefaultOpen : ImGuiTreeNodeFlags_None)) {
            ...
            {
                char locked_hdr[96];
                snprintf(locked_hdr, sizeof(locked_hdr), "%s  (%zu)", translationLocked[current_language], locked_idx.size());
                if (ImGui::CollapsingHeader(locked_hdr, settings->overlay_appearance.locked_expanded ? ImGuiTreeNodeFlags_DefaultOpen : ImGuiTreeNodeFlags_None)) {
                    if (locked_idx.empty()) {
        ...
        // NOTE: the settings button was removed from the main window per user request;
        // FPS/Frametime/Playtime toggles and the test-achievement button now live in
        // the header row above instead. This settings window (username/language) is
        // no longer reachable from the UI, left in place but effectively dead code.
        if (show_settings) {
            ImGui::SetNextWindowBgAlpha(1.0f);
