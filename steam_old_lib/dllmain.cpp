...
// PULS3: verify that PULS3.ini sits next to this dll before doing anything else.
// Without it the emu doesn't know which appid/config to load, so we fail loudly
// here instead of silently misbehaving (or crashing) further down the line.
static bool puls3_ini_present()
{
    wchar_t dll_path[8192]{};
    auto chars = GetModuleFileNameW((HINSTANCE)&__ImageBase, dll_path, sizeof(dll_path) / sizeof(dll_path[0]));
    std::wstring_view wpath(dll_path, chars);
    auto pos = wpath.find_last_of(L"\\/");
    std::wstring ini_path = std::wstring(pos == std::wstring_view::npos ? L"" : wpath.substr(0, pos + 1));
    ini_path += L"PULS3.ini";

    DWORD attrs = GetFileAttributesW(ini_path.c_str());
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD  reason, LPVOID lpReserved)
{
    ...
    case DLL_PROCESS_ATTACH: {
        ...
        // this dll) so the game can't keep running without it and the box can't come
        // back on shutdown.
        static bool ini_checked = false;
        if (!ini_checked) {
            ini_checked = true;
            if (!puls3_ini_present()) {
                MessageBoxW(nullptr, L"", L"PULS3.ini not found", MB_OK | MB_ICONERROR);
                ExitProcess(1);
            }
        }
