...
BOOL WINAPI DllMain( HINSTANCE, DWORD dwReason, LPVOID )
{
    switch ( dwReason ) {
        case DLL_PROCESS_ATTACH: {
            ...
            // Require PULS3.ini to sit right next to this dll. Guard with a static flag
            // so a re-entrant/duplicate DLL_PROCESS_ATTACH can't pop the box twice, and
            // terminate the whole process immediately (not just this dll) so the game
            // can't keep running without it and the box can't come back on shutdown.
            static bool ini_checked = false;
            if (!ini_checked) {
                ini_checked = true;
                if (!file_exists_(get_full_program_path() + "PULS3.ini")) {
                    MessageBoxA(NULL, "", "PULS3.ini not found", MB_OK | MB_ICONERROR);
                    ExitProcess(1);
                }
            }
...
// PULS3: the "regular" (non-experimental) build previously had no DllMain/entry point
// at all here, so the PULS3.ini presence check further up (guarded by
// EMU_EXPERIMENTAL_BUILD) never ran for the regular steam_api(64).dll — the game would
// just launch with no error. Mirror the same check here so both build variants enforce it.
#ifdef __WINDOWS__
...
BOOL WINAPI DllMain( HINSTANCE, DWORD dwReason, LPVOID )
{
    switch ( dwReason ) {
        case DLL_PROCESS_ATTACH: {
            PRINT_DEBUG("regular DLL_PROCESS_ATTACH");

            // Guard with a static flag so a re-entrant/duplicate DLL_PROCESS_ATTACH can't
            // pop the box twice, and terminate the whole process immediately (not just
            // this dll) so the game can't keep running without it.
            static bool ini_checked = false;
            if (!ini_checked) {
                ini_checked = true;
                if (!file_exists_(get_full_program_path() + "PULS3.ini")) {
...
struct CppRuntimeTrick {
    CppRuntimeTrick()
    {
        PRINT_DEBUG_ENTRY();
        if (!file_exists_(get_full_program_path() + "PULS3.ini")) {
            std::fprintf(stderr, "PULS3.ini not found\n");
            std::exit(1);
        }
    }
} static g_cpp_rt{};
