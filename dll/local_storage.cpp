...
std::string Local_Storage::saves_folder_name = "PULS3";
...
std::string Local_Storage::get_game_settings_path()
{
    // PULS3: no separate "steam_settings" folder, everything (PULS3.ini, achievements.json, etc.)
    // lives right next to the dll.
...
std::string Local_Storage::get_user_appdata_path()
{
    ...
    // PULS3: saves go under the public, non-elevated "Public Documents\Steam\PULS3\<appid>"
    // folder (same convention some other release groups use) instead of per-user AppData.
    // CSIDL_COMMON_DOCUMENTS is resolved dynamically via the shell API, so this correctly
    // follows whatever drive/path Windows (or a sysadmin/group policy folder redirection)
    // actually has Public Documents pointed at -- nothing here is a hardcoded "C:\...".
    HRESULT hr = SHGetFolderPathW(NULL, CSIDL_COMMON_DOCUMENTS, NULL, 0, szPath);

    if (SUCCEEDED(hr)) {
        user_appdata_path = utf8_encode(szPath);
        user_appdata_path.append(PATH_SEPARATOR).append("Steam");
    }
