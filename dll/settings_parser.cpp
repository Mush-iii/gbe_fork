...
// PULS3: single config file, sitting right beside the dll (== get_program_path() == get_game_settings_path())
constexpr const static char puls3_ini_file[]     = "PULS3.ini";
// PULS3: hardcoded, not user-configurable via PULS3.ini
constexpr const static char puls3_hardcoded_steamid[] = "76561197960287930";
// PULS3: [emu] appid override from PULS3.ini, filled in by load_puls3_ini(),
// consulted by parse_steam_app_id() before falling back to steam_appid.txt
static uint32 puls3_ini_appid = 0;
...
// PULS3: disabled on purpose. The stock behavior here reads/writes a shared
// "settings/configs.user.ini" file next to the per-appid save folders (account_name,
// language, ip_country, steamid, etc. all default-and-persist through this). PULS3
// builds only ever want values from PULS3.ini or the hardcoded defaults in settings.h,
// so this is now a no-op: nothing gets written, no "settings" folder is ever created,
// and every value is simply recomputed fresh (from PULS3.ini/defaults) on each launch.
...
static uint32 parse_steam_app_id(const std::string &program_path)
{
    ...
    // PULS3: [emu] appid from PULS3.ini, before falling back to steam_appid.txt files
    if (!appid && puls3_ini_appid) {
        appid = puls3_ini_appid;
        PRINT_DEBUG("using appid %u from PULS3.ini", appid);
    }
...
// shared line-by-line parser, used both for a standalone steam_interfaces.txt file
// and for the [interfaces] section extracted directly out of PULS3.ini
static void parse_old_steam_interfaces_stream(std::istream &input)
{
...
static bool try_parse_old_steam_interfaces_file(const std::string &interfaces_path)
{
    ...
    parse_old_steam_interfaces_stream(input);
    return true;
}

// PULS3: pull the [interfaces] section straight out of PULS3.ini (plain interface-name lines,
// same format as the classic steam_interfaces.txt, just embedded under a section header)
static bool try_parse_puls3_ini_interfaces_section(const std::string &puls3_ini_path)
{
    std::ifstream input( std::filesystem::u8path(puls3_ini_path) );
    if (!input.is_open()) return false;

    bool in_section = false;
    std::ostringstream section_lines{};
    for (std::string line; std::getline(input, line); ) {
        std::string trimmed = common_helpers::string_strip(line);
        if (!trimmed.empty() && trimmed.front() == '[' && trimmed.back() == ']') {
            in_section = common_helpers::str_cmp_insensitive(trimmed.c_str(), "[interfaces]");
            continue;
        }
        if (in_section && !trimmed.empty()) {
            section_lines << trimmed << "\n";
        }
    }

    std::string collected = section_lines.str();
    if (collected.empty()) return false;

    PRINT_DEBUG("Trying to parse [interfaces] section from '%s'", puls3_ini_path.c_str());
    std::istringstream stream(collected);
    parse_old_steam_interfaces_stream(stream);
    return true;
}
...
static void parse_old_steam_interfaces()
{
    ...
    if (!try_parse_puls3_ini_interfaces_section(program_path + puls3_ini_file) &&
        !try_parse_old_steam_interfaces_file(Local_Storage::get_game_settings_path() + "steam_interfaces.txt") &&
        !try_parse_old_steam_interfaces_file(program_path + "steam_interfaces.txt")) {
        PRINT_DEBUG("Couldn't load interfaces from PULS3.ini or steam_interfaces.txt");
    }
}

// PULS3: translate PULS3.ini's simple [emu]/[dlcs] sections into the internal
// section::key layout the rest of the parser already understands, so none of the
// existing per-feature parse functions below need to change.
static void load_puls3_ini()
{
    const std::string puls3_ini_path(Local_Storage::get_program_path() + puls3_ini_file);

    CSimpleIniA puls3_ini{};
    puls3_ini.SetUnicode();
    puls3_ini.SetSpaces(false);

    std::ifstream puls3_ini_file_stream( std::filesystem::u8path(puls3_ini_path), std::ios::binary | std::ios::in);
    if (!puls3_ini_file_stream.is_open()) {
        PRINT_DEBUG("Couldn't open '%s'", puls3_ini_path.c_str());
    } else {
        auto err = puls3_ini.LoadData(puls3_ini_file_stream);
        puls3_ini_file_stream.close();
        PRINT_DEBUG("result of parsing '%s' %i (success == 0)", puls3_ini_path.c_str(), (int)err);
    }

    // [emu] username -> user::general::account_name
    {
        auto username = puls3_ini.GetValue("emu", "username", "");
        if (username && username[0]) {
            ini.SetValue("user::general", "account_name", username);
        }
    }

    // [emu] language -> user::general::language
    {
        auto language = puls3_ini.GetValue("emu", "language", "");
        if (language && language[0]) {
            ini.SetValue("user::general", "language", language);
        }
    }

    // account_steamid: always hardcoded, never read from PULS3.ini
    ini.SetValue("user::general", "account_steamid", puls3_hardcoded_steamid);

    // [emu] appid -> used by parse_steam_app_id() as a fallback so the
    // overlay/game don't show appid 0 when there's no steam_appid.txt
    {
        auto appid_str = puls3_ini.GetValue("emu", "appid", "");
        if (appid_str && appid_str[0]) {
            try {
                puls3_ini_appid = std::stoul(appid_str);
            } catch (...) {
                puls3_ini_appid = 0;
                PRINT_DEBUG("invalid [emu] appid value in PULS3.ini: '%s'", appid_str);
            }
        }
    }

    // [emu] overlay -> overlay::general::enable_experimental_overlay
    // (everything else overlay-related is hardcoded in settings.h defaults)
    {
        bool overlay_enabled = puls3_ini.GetBoolValue("emu", "overlay", true);
        ini.SetValue("overlay::general", "enable_experimental_overlay", overlay_enabled ? "true" : "false");
    }

    // [dlcs] -> app::dlcs (same key layout, copied verbatim)
    {
        bool unlock_all = puls3_ini.GetBoolValue("dlcs", "unlock_all", false);
        ini.SetValue("app::dlcs", "unlock_all", unlock_all ? "true" : "false");

        std::list<CSimpleIniA::Entry> dlcs_keys{};
        if (puls3_ini.GetAllKeys("dlcs", dlcs_keys)) {
            dlcs_keys.remove_if([](const CSimpleIniA::Entry &item){
                return common_helpers::str_cmp_insensitive(item.pItem, "unlock_all");
            });

            for (const auto &dlc_key : dlcs_keys) {
                auto val = puls3_ini.GetValue("dlcs", dlc_key.pItem, "");
                ini.SetValue("app::dlcs", dlc_key.pItem, val);
            }
        }
    }

    // new_app_ticket / gc_token are hardcoded (Settings::enable_new_app_ticket /
    // Settings::use_gc_token already default to true and [main::general] is never populated)

    // [interfaces] is handled separately in parse_old_steam_interfaces()
}
...
static void load_all_config_settings()
{
    ...
    // PULS3: single PULS3.ini beside the dll, no steam_settings folder, no local-save
    // or appdata config layering.
    load_puls3_ini();
